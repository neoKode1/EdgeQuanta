"""
Superconducting quantum system protocol adapter (超导通信协议)
"""
import json
...
class SuperconductingAdapter:
    """Adapter for superconducting quantum system protocol"""
    
    @staticmethod
    def parse_general_message(msg: str) -> Dict[str, Any]:
        """Parse general message (flat structure for superconducting)"""
        data = json.loads(msg)
        ...
        # This makes parse_general_message return all fields like parse_task_message does
        result.update(data)
    ...
    @staticmethod
    def build_heartbeat_ack(sn: int, chip_id: int, topic: str) -> str:
    ...
    @staticmethod
    def build_task_result(sn: int, task_id: str, keys: list, prob_counts: list, 
                      note_time: Dict[str, int], err_code: int = 0, 
                      err_info: str = "") -> str:
